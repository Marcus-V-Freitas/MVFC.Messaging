...
public sealed class ServiceBusIntegrationTests(ServiceBusFixture fixture, ITestOutputHelper output) : IClassFixture<ServiceBusFixture>
{
    ...
    public async Task Should_PublishAndConsume_SingleMessage()
    {
        ...
        await using var publisher = new ServiceBusPublisher<TestMessage>(ServiceBusFixture.ConnectionString, queueName);
        await using var consumer = new ServiceBusConsumer<TestMessage>(ServiceBusFixture.ConnectionString, queueName);
    ...
    {
        // Arrange - usar a mesma fila pré-configurada
        const string queueName = "queue.1";
