...
public sealed class KafkaIntegrationTests(KafkaFixture fixture, ITestOutputHelper output) : IClassFixture<KafkaFixture>
{
    ...
    public async Task Should_PublishAndConsume_SingleMessage()
    {
        ...
        await using var publisher = new KafkaPublisher<TestMessage>(_fixture.BootstrapServers, topic);
        await using var consumer = new KafkaConsumer<TestMessage>(_fixture.BootstrapServers, topic, groupId);
